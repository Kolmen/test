...
![MLC window: job done](./images/done.png)
...
Please note that the UCF file, after it is generated by the Unico-GUI, always contains a line that configures the routing of interrupt status bit for decision tree 1 (MLC1) to  INT1 pin (**INT1_MLC1** bit of **MLC_INT1 (0Dh)** register is set to 1).

If the interrupt status bits for both decision trees should be routed to pin INT1, then the UCF file must be modified. This can be done simply with a text editor by searching for the line `Ac 0D 01` and replacing it with `Ac 0D 03` (write 0x03 instead of 0x01 to 0x0D [MLC_INT1] - see the [data sheet of the IIS2ICLX](https://www.st.com/resource/en/datasheet/iis2iclx.pdf) for more information on sensor configuration):
...
- Decision Tree Attributes:
	...
	- Results for both, DT1 and DT2 (the results are the same if no modification of the program is made): 
